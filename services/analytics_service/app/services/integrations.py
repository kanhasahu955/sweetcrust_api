"""Razorpay / ImageKit credential checks + thin gateway helpers (admin-owned)."""
...
def check_razorpay() -> dict[str, Any]:
    s = get_settings()
    if not s.razorpay_configured:
...
def check_imagekit() -> dict[str, Any]:
    s = get_settings()
...
def credentials_check() -> dict[str, Any]:
    return {"razorpay": check_razorpay(), "imagekit": check_imagekit()}
...
def create_razorpay_order(*, amount_inr: float, receipt: str, notes: Optional[dict] = None) -> dict:
    paise = max(100, int(round(float(amount_inr) * 100)))
    body = {"amount": paise, "currency": "INR", "receipt": receipt[:40], "notes": notes or {}}
    with httpx.Client(timeout=30.0) as client:
