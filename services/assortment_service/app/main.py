"""Assortment FastAPI app — async OOP."""
...
logger = get_logger("assortment")
boot_settings()

async def _startup() -> None:
    init_db()
    logger.info("assortment_service ready (async-oop)")

@asynccontextmanager
async def lifespan(app: FastAPI):
    boot_settings()
    async with service_lifespan(app, service="assortment", version=get_settings().service_version, on_startup=_startup):
        yield

def create_app() -> FastAPI:
    boot_settings()
    app = create_service_app(
        title="SweetCrust Assortment",
        version=get_settings().service_version,
        description="Range / draft / active (async OOP)",
        lifespan=lifespan,
    )
    routes.mount(app)
