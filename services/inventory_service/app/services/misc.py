...
def inventory(session: Session):
    current = list(session.exec(select(Product).order_by(Product.brand_name, Product.name)).all())
    low = [p for p in current if p.stock_status == StockStatus.LOW_STOCK]
    out = [p for p in current if p.stock_status == StockStatus.OUT_OF_STOCK]
    return {"current": current, "low_stock": low, "out_of_stock": out}
...
def list_chats(session: Session):
    return list(session.exec(select(Conversation).order_by(Conversation.updated_at.desc()).limit(100)).all())
