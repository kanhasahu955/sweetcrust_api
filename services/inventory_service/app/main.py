"""Inventory FastAPI app — async OOP."""
...
async def _startup() -> None:
    init_db()
    logger.info("inventory_service ready (async-oop)")

@asynccontextmanager
async def lifespan(app: FastAPI):
    boot_settings()
    async with service_lifespan(app, service="inventory", version=get_settings().service_version, on_startup=_startup):
        yield

def create_app() -> FastAPI:
    boot_settings()
    app = create_service_app(
        title="SweetCrust Inventory",
        version=get_settings().service_version,
        description="Stock (async OOP)",
        lifespan=lifespan,
    )
    routes.mount(app)
