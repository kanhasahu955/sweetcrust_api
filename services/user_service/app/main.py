...
async def _startup() -> None:
    init_db()
    logger.info("user_service ready (async-oop)")


@asynccontextmanager
async def lifespan(app: FastAPI):
    boot_settings()
    async with service_lifespan(app, service="user", version=get_settings().service_version, on_startup=_startup):
        yield
...
def create_app() -> FastAPI:
    ...
    app = create_service_app(
        title="SweetCrust User",
        version=get_settings().service_version,
        description="Profile /me (async OOP)",
        lifespan=lifespan,
    )
    routes.mount(app)
