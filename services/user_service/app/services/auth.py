...
def retailer_google_callback_uri(_api_base: str | None = None) -> str:
    settings = get_settings()
    base = (settings.auth_public_base_url or "http://127.0.0.1:8000").rstrip("/")
...
def retailer_google_oauth_setup(api_base: str | None = None) -> dict:
    ...
    return {
        ...
        "authorized_redirect_uris_to_add": [
            "http://127.0.0.1:8000/api/v1/auth/retailer/google/callback",
            "http://localhost:8000/api/v1/auth/retailer/google/callback",
        ],
        "lan_ip_not_allowed": lan_warning,
        "phone_testing": (
            "On a physical phone, run a tunnel so Google gets a public https redirect, e.g. "
            "`npx cloudflared tunnel --url http://127.0.0.1:8000` then set AUTH_PUBLIC_BASE_URL "
        ...
        "checklist": [
            ...
            "Credentials → Web application client only",
            f"Add Authorized redirect URI: {callback}",
            "Also add http://localhost:8000/api/v1/auth/retailer/google/callback",
            "OAuth consent screen → Testing → add your Gmail as Test user",
            "Simulator/Emulator: AUTH_PUBLIC_BASE_URL=http://127.0.0.1:8000",
