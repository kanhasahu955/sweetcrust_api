...
async def _startup() -> None:
    init_db()
    logger.info("auth_service ready (async-oop)")

@asynccontextmanager
async def lifespan(app: FastAPI):
    boot_settings()
    async with service_lifespan(app, service="auth", version=get_settings().service_version, on_startup=_startup):
        yield

def create_app() -> FastAPI:
    boot_settings()
    app = create_service_app(
        title="SweetCrust Auth",
        version=get_settings().service_version,
        description="OTP, JWT, Google, roles (async OOP)",
        lifespan=lifespan,
    )
    routes.mount(app)
