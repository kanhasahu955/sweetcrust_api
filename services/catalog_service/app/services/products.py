...
def resolve_brand_fields(
    ...
    *,
    ...
    """Fill brand from wholesaler shop name when supplier is set. Pure helper (testable)."""
...
def invalidate_catalog_cache() -> None:
    ...
    redis_delete_prefix(BRANDS_KEY)
...
def list_brands(session: Session) -> list[dict]:
    ...
    if cached:
        try:
            ...
            pass
    rows = session.exec(
        select(Product.brand_name).where(
            Product.is_active == True,  # noqa: E712
            Product.is_draft == False,  # noqa: E712
        )
    ).all()
    names = sorted({str(n).strip() for n in rows if n and str(n).strip()})
    brands = [{"name": n} for n in names]
    redis_set(BRANDS_KEY, json.dumps(brands), HOME_FEED_TTL)
    return brands
