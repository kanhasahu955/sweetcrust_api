...
logger = get_logger("picking")
boot_settings()

async def _startup() -> None:
    init_db()
    logger.info("picking_service ready (async-oop)")

@asynccontextmanager
async def lifespan(app: FastAPI):
    boot_settings()
    async with service_lifespan(app, service="picking", version=get_settings().service_version, on_startup=_startup):
        yield

def create_app() -> FastAPI:
    boot_settings()
    app = create_service_app(
        title="SweetCrust Picking",
        version=get_settings().service_version,
        description="Kitchen queue (async OOP)",
        lifespan=lifespan,
    )
    routes.mount(app)
