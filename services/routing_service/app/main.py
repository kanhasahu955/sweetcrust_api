...
async def _startup() -> None:
    init_db()
    logger.info("routing_service ready (async-oop)")

@asynccontextmanager
async def lifespan(app: FastAPI):
    boot_settings()
    async with service_lifespan(app, service="routing", version=get_settings().service_version, on_startup=_startup):
        yield

def create_app() -> FastAPI:
    boot_settings()
    app = create_service_app(
        title="SweetCrust Routing",
        version=get_settings().service_version,
        description="Route optimize & ETA (async OOP)",
        lifespan=lifespan,
    )
    routes.mount(app)
