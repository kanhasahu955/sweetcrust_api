...
async def _startup() -> None:
    init_db()
    logger.info("cart_service ready (async-oop)")

@asynccontextmanager
async def lifespan(app: FastAPI):
    boot_settings()
    async with service_lifespan(app, service="cart", version=get_settings().service_version, on_startup=_startup):
        yield

def create_app() -> FastAPI:
    boot_settings()
    app = create_service_app(
        title="SweetCrust Cart",
        version=get_settings().service_version,
        description="Cart (async OOP)",
        lifespan=lifespan,
    )
    routes.mount(app)
