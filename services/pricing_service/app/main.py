...
async def _startup() -> None:
    init_db()
    logger.info("pricing_service ready (async-oop)")

@asynccontextmanager
async def lifespan(app: FastAPI):
    boot_settings()
    async with service_lifespan(app, service="pricing", version=get_settings().service_version, on_startup=_startup):
        yield

def create_app() -> FastAPI:
    boot_settings()
    app = create_service_app(
        title="SweetCrust Pricing",
        version=get_settings().service_version,
        description="Quotes & price rules (async OOP)",
        lifespan=lifespan,
    )
    routes.mount(app)
